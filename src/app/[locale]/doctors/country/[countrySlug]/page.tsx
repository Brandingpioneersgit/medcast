...
export async function generateMetadata({ params }: Props) {
  ...
  return generateMeta({
    title: `Top doctors in ${countrySlug}`,
    description: `Browse leading specialists in ${countrySlug} — experience, ratings, and consultation fees.`,
    path: `/doctors/country/${countrySlug}`,
    locale,
  });
}

export default async function DoctorsByCountryPage({ params }: Props) {
  const { locale, countrySlug } = await params;
    ...
    <section className="py-14 md:py-18">
      <div className="mx-auto w-full max-w-[90rem] px-5 md:px-8">
        ...
        <p className="lede mt-4 max-w-[44rem]">
          Top specialists practicing in this destination.
        </p>
      </div>
    </section>
  );
